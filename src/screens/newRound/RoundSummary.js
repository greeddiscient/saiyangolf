...
import { Text, Container, Content, StyleProvider, Input } from "native-base";
...
class RoundSummaryScreen extends React.Component {
  ...
  calculateGIR() {
    ...
    for (var i = 0; i < roundSummary.length; i++) {
      if (roundSummary[i].par == 3 && roundSummary[i].shots.length == 1) {
        ...
        roundSummary[i].par == 4 &&
        roundSummary[i].shots.length <= 2
      ) {
        gir += 1;
      } else if (
        roundSummary[i].par == 5 &&
        roundSummary[i].shots.length <= 3
      ) {
        gir += 1;
      }
    }
  ...
  calculateFairways() {
    ...
    for (var i = 0; i < roundSummary.length; i++) {
      if (roundSummary[i].par == 4 && roundSummary[i].shots.length == 1) {
        ...
        roundSummary[i].par == 4 &&
        roundSummary[i].shots[1].lie == "F"
      ) {
        fairways += 1;
      } else if (
        roundSummary[i].par == 5 &&
        (roundSummary[i].shots[1] != null &&
          roundSummary[i].shots[1].lie != null &&
          roundSummary[i].shots[1].lie == "F")
      ) {
        fairways += 1;
      }
  ...
  calculateDrivingStrokesGained() {
    ...
    for (var i = 0; i < roundSummary.length; i++) {
      if (roundSummary[i].par == 3) {
        ...
        var drivingSGNew =
          roundSummary[i].shots[0].sg != null ||
          roundSummary[i].shots[0].sg != NaN
            ? roundSummary[i].shots[0].sg
            : 0;
  ...
  calculateWedgeStrokesGained() {
    ...
    for (var i = 0; i < roundSummary.length; i++) {
      for (var j = 0; j < roundSummary[i].shots.length; j++) {
        if (
          roundSummary[i].shots[j].distance <= 125 &&
          roundSummary[i].shots[j].distance > 50 &&
          roundSummary[i].shots[j].lie != "S" &&
          roundSummary[i].shots[j].lie != "RC"
        ) {
  ...
  async saveAPI() {
    if (this.valueChecker() == true && this.state.visibleLoading == false) {
      ...
      const dataSend = {
        drivingDistance: isNaN(this.state.drivingDistance)
          ? 0
          : this.state.drivingDistance,
        ...
        totalPuttingSG: isNaN(this.state.totalPuttingSG)
          ? 0
          : this.state.totalPuttingSG,
      ...
      console.log(
        "data send to server in round summary, " + JSON.stringify(dataSend)
      );
        ...
        {roundSummary.map((data, i) => {
          return (
            <View key={i} style={styles.cardRS}>
                ...
                return (
                  <View key={j} style={{ height: 40, marginTop: 20 }}>
                    ...
                    >
                      ...
                      <Text style={styles.baseTextRS}>
                        {dataShot.distance}
                        {dataShot.lie} SG : {dataShot.sg}
                      </Text>
                    </View>
                    <View
                      style={{
              ...
              <Text style={styles.baseTextRS}>
                Putting SG : {data.puttingSG}
              </Text>
              <Text style={styles.baseTextRS}>
                Putting Distance : {data.distancePutt}
              </Text>
              <Text style={styles.baseTextRS}>
                Driving Distance : {data.drivingDistance}
              </Text>
      ...
      <StyleProvider style={getTheme(material)}>
        <Container style={{ overflow: "hidden" }}>
                ...
                <View style={{ width: width - 60, marginLeft: 30 }}>
                  ...
                  <View>
                    <View style={{ flexDirection: "row" }}>
                      <Text style={styles.textSubTitleRHD}>
                        Driving distance :{" "}
                      </Text>
                      <Text style={styles.textSubTitleRightRHD}>
                        {this.state.drivingDistance}
                      </Text>
                    </View>
                    <View style={{ flexDirection: "row" }}>
          ...
          >
            ...
            <TouchableOpacity
              style={styles.containerBackImage}
            ...
            <View style={styles.containerContentModalRS}>
              <View style={{ justifyContent: "center", borderBottomWidth: 0 }}>
                <View>
                  ...
                  <Text style={styles.textSubTitleRightRHD}>
                    Round has been successfully saved.
                  </Text>
                </View>
              </View>
    ...
    listData.map((data, i) => {
      listBox.push(
        <View
          key={i}
          style={[
            styles.containerBoxHorizontal,
            {
              marginRight: data.id == 3 || data.id == 6 ? 0 : 10
            }
          ]}
        >
          <Text style={[styles.textTitleSmallBoxRHD, { fontSize: 14 }]}>
            {data.value}
          </Text>
          <Text style={[styles.textSmallBoxRHD, { fontSize: 10 }]}>
            {data.name}
          </Text>
        </View>
      );
    });
    return <View style={styles.subLittleBoxContainer}>{listBox}</View>;
  }
